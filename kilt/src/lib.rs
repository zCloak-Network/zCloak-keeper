...
use keeper_primitives::{
	kilt::{get_attestation_storage_key, Attestation, Error, KiltClient, KILT_MAX_RETRY_TIMES},
	Decode, Hash, VerifyResult,
};
...
pub async fn filter(client: &KiltClient, result: Vec<VerifyResult>) -> Result<Vec<VerifyResult>> {
	let mut v = vec![];
	for i in result {
		let r = query_attestation(client, i.root_hash.into())
			.await
