...
use server_traits::server::{component::ServerComponent, types::WebServerState};
use task_management::{
	config::{MicrokvConfig, TaskManagementConfig},
	resource::TaskManagementComponent,
};

use crate::{
	route,
	route::task_manager,
	utils::{command::ServerOptions, server::Resp, utils},
};

/// Handler zCloak server
...
async fn app_state(options: ServerOptions) -> anyhow::Result<WebServerState> {
	...
	Ok(WebServerState { base_path: Arc::new(base_path) })
}

/// Routerify access log
async fn logger(req: Request<Body>) -> anyhow::Result<Request<Body>> {
	log::debug!("{} {} {}", req.remote_addr(), req.method(), req.uri().path());
	Ok(req)
}
