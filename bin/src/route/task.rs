...
use server_traits::{error::StandardError, server::types::WebServerState};
...
pub async fn task_list(_req: Request<Body>) -> anyhow::Result<Response<Body>> {
	...
	let data = tasks
		...
		.map(|item| {
			...
			TaskListResponse { name: item.clone(), running }
		})
...
pub async fn task_route(mut req: Request<Body>) -> anyhow::Result<Response<Body>> {
	let param: serde_json::Value =
		utils::deserialize_body(&mut req).await.unwrap_or(serde_json::Value::Null);
	...
	let task = task_management::task::running_task(task_name).ok_or_else(|| {
		StandardError::Api(format!("The task [{}] not found or isn't started", task_name))
	})?;
