use keeper_primitives::{
    Address, Contract, Events, Http, ProofEvent,
...
use crate::types::{MoonbeamClient, IS_FINISHED, MOONBEAM_LISTENED_EVENT, MOONBEAM_SUBMIT_LOG_TARGET,
                   MOONBEAM_SCAN_SPAN, MOONBEAM_TRANSACTION_CONFIRMATIONS, MOONBEAM_SCAN_LOG_TARGET,
                   SUBMIT_STATUS_QUERY, SUBMIT_TX_MAX_RETRY_TIMES, SUBMIT_VERIFICATION
};
use crate::utils::query_submit_and_finish_result;

// scan moonbeam events
// return value:
// Ok(Some(event)) -> hit some events
// Ok(None) -> no event hit while no error
pub async fn scan_events(
...
) -> MoonbeamResult<(Option<Events>, U64)> {
    ...
    let r = timeout_at(
        ...
        super::utils::events::<_, ProofEvent>(
            client.eth(),
            proof_contract,
...
pub async fn submit_txs(
    ...
    // todo: move outside?
    let keeper_pri_ref: SecretKeyRef = keeper_pri.into();
    let keeper_address = keeper_pri_ref.address();
    for v in res {
