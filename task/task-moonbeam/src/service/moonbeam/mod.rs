...
impl Service for MoonBeamService {
	...
	fn spawn(bus: &Self::Bus) -> Self::Lifeline {
		...
		// TODO this start can from the cli command;
		let start = None;
		...
		let _greet = Self::try_task(&format!("{}-service-task", MoonbeamTask::NAME), async move {
			while let Some(message) = rx.recv().await {
				...
				match message {
					MoonbeamTaskMessage::Start => {
						tokio::spawn(async move {
							run::run_worker(start, moonbean_config, contract_config, ipfs_config, kilt_config).await
						});
					}
					// MoonbeamTaskMessage::ListenMoonbeam => {
					// 	listen_moonbeam::listen_moonbeam_event().await;
					// 	tokio::spawn(async move {
					// 		run_subscribe(
					// 			url, address, ipfs_url, password, keystore, kilt_url, seed,
					// 		)
					// 		.await
					// 	});
					...
					// 	// let keystore = contract_config.keystore.clone();
					...
					// 	// log::info!("moonbeam server is running")
					// },
					// MoonbeamTaskMessage::IpfsProof(_) => {}
					// MoonbeamTaskMessage::KiltAttestation(_) => {}
