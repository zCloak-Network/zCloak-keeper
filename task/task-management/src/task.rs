...
use server_traits::{error::StandardError, server::task::ServerTaskKeep};

static AVAILABLE_TASKS: Lazy<Mutex<Vec<String>>> = Lazy::new(|| Mutex::new(vec![]));

static mut RUNNING_TASKS: OnceCell<HashMap<String, Box<dyn ServerTaskKeep + Send + Sync>>> =
	OnceCell::new();
...
pub fn running_task(
	name: impl AsRef<str>,
) -> Option<&'static Box<dyn ServerTaskKeep + Send + Sync>> {
	let name = name.as_ref();
	unsafe {
...
pub fn running_task_downcast_ref<T: 'static + ServerTaskKeep>(
	name: impl AsRef<str>,
...
pub fn running_task_downcast_mut<T: 'static + ServerTaskKeep>(
	name: impl AsRef<str>,
