...
use substrate_subxt::{
    ...
    PairSigner,EventsDecoder,
};
use sp_keyring::AccountKeyring;
use starksVM as stark;
use primitives::frame::verify::{UserTaskCreatedEvent, ClientSingleReponseCall, ClientSingleReponseCallExt};
use primitives::frame::sudo::SudoCallExt;
...
use crate::account::ZcloakAccount;
            ...
            match raw_event {
                Ok(r) => {
                    ...
                    if let Ok(e) = UserTaskCreatedEvent::<ZcloakRuntime>::decode(&mut &r.data[..]){
                        ...
                        let stark_proof = bincode::deserialize::<stark::StarkProof>(&proof).map_err(|_| StandardError::Other("verifier deserialize error".to_string()))?;

                        let is_success = stark::verify(&programhash, &inputs, &outputs, &stark_proof);

                        let res = if let Ok(r) = is_success {
                                log::debug!("proofid {:?} stark verify true ---", str::from_utf8(&proofid).unwrap()); 
                                r 
                            } else { 
                                log::debug!("proofid {:?} stark verify false --", str::from_utf8(&proofid).unwrap()); 
                                false 
                            };
