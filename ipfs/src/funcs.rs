...
use super::types::{IpfsClient, IPFS_LOG_TARGET};
use keeper_primitives::{Events};

// empty return is set to none
pub async fn query_and_verify(
    ipfs: &IpfsClient,
    input: Events,
) -> Result<Option<Vec<VerifyResult>>> {
...
pub(crate) fn verify(p: &ProofEvent, context: &[u8]) -> std::result::Result<bool, Error> {
