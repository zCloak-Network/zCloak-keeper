...
mod tests {
	...
	async fn verify_should_work() {
		let program_hash = [85,240,19,237,111,148,74,101,238,76,236,4,253,175,28,149,160,161,81,162,117,180,36,64,29,56,109,193,196,236,207,254];
		let proof_cid = "QmRFeY7ZeywFyXzT7pCR9ZGyZqhNs9y4ozhMGgSpvTAb4f";
		let public_inputs = [6383461,427020088179,8271117968073418672650679055481,30765223346328968342731846777,9459527121954502519414720132217];
		let public_outputs = [138393280564113376992984738626752023914,281318377845317858458548900585840899268,1,0,0];

		let ipfs_client = IpfsClient::new("https://ipfs.infura.io:5001").expect("ipfs client building fails.");
